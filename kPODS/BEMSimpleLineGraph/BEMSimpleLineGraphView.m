...
@interface BEMSimpleLineGraphView () {
    ...
    NSMutableArray *secondaryDataPoints;
...
- (void)commonInit {
    ...
    secondaryDataPoints = [NSMutableArray array];
}
...
- (void)layoutTouchReport {
    ...
    if (self.enableTouchReport == YES || self.enablePopUpReport == YES) {
        ...
        if (self.enablePopUpReport == YES && self.alwaysDisplayPopUpLabels == NO) {
            ...
            NSString *maxValueString = [NSString stringWithFormat:@"bpm: %li\nSpO2: %li",
                                        (long)[self calculateMaximumPointValue].integerValue,
                                        (long)[self calculateMaximumPointValue2].integerValue];
            NSString *minValueString = [NSString stringWithFormat:@"bpm: %li\nSpO2: %li",
                                        (long)[self calculateMinimumPointValue].integerValue,
                                        (long)[self calculateMinimumPointValue2].integerValue];
...
- (void)drawDots {
    ...
    [secondaryDataPoints removeAllObjects];
...
#pragma clang diagnostic pop
            ...
            [secondaryDataPoints addObject:[NSNumber numberWithFloat:[self.dataSource lineGraph:self valueForPointAtIndex2:i]]];
...
- (NSNumber *)calculateMinimumPointValue2 {
    if (secondaryDataPoints.count > 0) {
        NSExpression *expression = [NSExpression expressionForFunction:@"min:" arguments:@[[NSExpression expressionForConstantValue:secondaryDataPoints]]];
        NSNumber *value = [expression expressionValueWithObject:nil context:nil];
        return value;
    } else return @0;
}

- (NSNumber *)calculateMaximumPointValue2 {
    NSExpression *expression = [NSExpression expressionForFunction:@"max:" arguments:@[[NSExpression expressionForConstantValue:secondaryDataPoints]]];
    NSNumber *value = [expression expressionValueWithObject:nil context:nil];
...
- (void)setUpPopUpLabelAbovePoint:(BEMCircle *)closestPoint {
    ...
    if ([self.delegate respondsToSelector:@selector(popUpSuffixForlineGraph:)])
        ...
        self.popUpLabel.text = [NSString stringWithFormat:@"bpm: %li\nSpO2: %li", (long)[dataPoints[(NSInteger) closestDot.tag - DotFirstTag100] integerValue], (long)[secondaryDataPoints[(NSInteger) closestDot.tag - DotFirstTag100] integerValue]];
